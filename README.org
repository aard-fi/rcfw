...
|       9 | FL1      | switch, 2 state | default 1000, expansion module (front) |
|      10 | FL2      | switch, 2 state | default 1000, expansion module (back)  |

The expansion module is inactive per default, to configure it:

- go to hardware settings, and set S3/S4 to =switch=
- scroll down, and set =FL1= to =S3=, =FL2= to =S4=, both with =2POS=
- go to model settings, mixes page, and add channel 9 with FL1, channel 10 with FL2
